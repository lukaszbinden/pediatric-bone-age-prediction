...
def execute():
    ...
    train_gen_chest, val_gen_chest = get_gen(train_idg, val_idg, IMG_SIZE, BATCH_SIZE_TRAIN, BATCH_SIZE_VAL, 'chest')
    train_gen_boneage, val_gen_boneage = get_gen(train_idg, val_idg, IMG_SIZE, BATCH_SIZE_TRAIN, BATCH_SIZE_VAL,
                                                 'boneage')

    model = get_model('winner', True, 'imagenet')

    history = train(train_gen_boneage, val_gen_boneage, model, OPTIMIZER, LOSS, LEARNING_RATE, NUM_EPOCHS,
